...
public class WorkflowController(IWorkflowAppService service, UserContext userContext) : BpmControllerBase
{
    ...
    [ProducesResponseType(typeof(Result<ListDefinitionsValueResponse>), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiMessage), StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(ApiMessage), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ApiMessage), StatusCodes.Status500InternalServerError)]
    public async Task<IActionResult> ListDefinitions([FromBody] ListDefinitionsRequest? request)
    {
        var categoria = request?.Filters?
            .SelectMany(x => x)
            .FirstOrDefault(x => x.Key.Equals("CATEGORY", StringComparison.OrdinalIgnoreCase))
            .Value;
        ...
        var response = new Result<ListDefinitionsValueResponse>
        {
            Success = result.Success,
            Value = new ListDefinitionsValueResponse
            {
                Items = result.Value?.Select(x => new ListDefinitionsItemResponse
                {
                    DefinitionType = x.ProcessType,
                    Name = x.Name,
                    Category = x.Category
                }).ToList() ?? []
            },
    ...
    /// <returns></returns>
    [HttpPost("search")]
    [ProducesResponseType(typeof(SearchProcessDto), StatusCodes.Status200OK)]
    [ProducesResponseType(typeof(ApiMessage), StatusCodes.Status403Forbidden)]
    [ProducesResponseType(typeof(ApiMessage), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ApiMessage), StatusCodes.Status500InternalServerError)]
