...
public class BpmnParserService : IBpmnParserService
{
    ...
    public WorkflowDefinition Parse(string xml)
    {
        ...
        var definitions = doc.Root ?? throw new Exception("Invalid BPMN XML");
        ...
        List<UserTaskField> globalFields)
    {
        ...
        actionTasks.Add(new ActionUserTaskDefinition
        {
            ...
            Confirm = new ActionDefinition(
                ...
                label: "Conferma",
                targetNodeId: "",
                requirements: Array.Empty<ActionRequirementDefinition>()),
    ...
    // SERVICE TASK
    // ------------------------------------------------------------
    private void ParseServiceTask(XElement element, List<TaskDefinition> systemTasks)
    {
        var id = element.Attribute("id")?.Value ?? Guid.NewGuid().ToString();
        var name = element.Attribute("name")?.Value ?? "";
    ...
    // GATEWAY
    // ------------------------------------------------------------
    private void ParseGateway(XElement element, List<GatewayDefinition> gateways)
    {
        var id = element.Attribute("id")?.Value ?? Guid.NewGuid().ToString();
        var name = element.Attribute("name")?.Value ?? "";
        ...
        gateways.Add(new GatewayDefinition(
            id: id,
            label: name,
            outgoing: new List<GatewayOutgoing>()
        ));
    }
    ...
    private List<UserTaskField> ParseForms(XElement element)
    {
        ...
        foreach (var ui in ext.Elements(zv + "ui"))
        {
